...
export function buildPrompt(opts: BuildPromptOptions): string {
	...
	return [
		...
		'## ツール一覧と仕様（必ず従う）',
		'',
		'- Cogent 本体が提供する Language Model ツール',
		...
		'',
		'cogent_createFile (CreateFileTool)\n  - 入力: { path: string, content?: string, overwrite?: boolean }\n  - 振る舞い: workspace.fs.writeFile を呼び、必要に応じて親ディレクトリを作成します。`path` は原則ワークスペースルートからの相対パスを推奨しますが、絶対パスを指定することも可能です（ただしワークスペース外のファイル作成はデフォルトで拒否され、許可するにはユーザーが `cogent.allowExternalFileCreation` を設定で有効化する必要があります）。ドライブルート（例: `C:\`）への直接作成は行わないため、無効なパス指定で mkdir エラーが発生するリスクは低減されています。\n  - 注意: 200 行を超える大規模なファイルやバイナリについては差分/分割戦略を検討してください。\n  - 例: { "path": "src/main/java/org/example/.../MiningBooster.java", "content": "...source...", "overwrite": false }',
		'',
		'',
