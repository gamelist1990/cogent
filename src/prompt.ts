...
export function buildPrompt(opts: BuildPromptOptions): string {
	...
	return [
		...
		'',
		'## ツールに関する明示的なルール',
		"- Cogent 本体が提供する Language Model ツールは次の通りです（実行時にランタイムで登録されているか必ず確認すること）：",
		...
		"- ただし、ツールが実際に利用可能かは実行時に検証すること（vscode.lm.tools の登録状況や権限によって利用不可の場合がある）。",
		"- ファイルの読み取り、検索、差分適用、コマンド実行、エディタ操作等は VS Code / Copilot の組み込みツールを優先して使用してください（例: workspace_read, workspace_search, applyDiff, runCommand, getVscodeApi 等）。",
		...
		"- cogent_getAbsolutePath (GetAbsolutePathTool): 入力 { path: string } — 相対パス/URI を絶対ファイルシステムパスに解決します。",
		"- cogent_removeFile (RemoveFileTool): 入力 { path: string, recursive?: boolean } — workspace.fs.delete を呼びファイル/ディレクトリを削除します（復元不可）。",
		"- 注意: 実行時にツールが登録・利用可能であるかを常に確認してください（vscode.lm.tools）。",
		'',
